namespace GtaV.TwitchPedSpawner.Client.Pages;
...
public partial class Home
{
        ...
        if (firstRender)
        {
            if (Code is not null)
            {
                try
                {
                    ...
                    if (json is not null)
                    {
                        await http.PostAsJsonAsync("http://127.0.0.1:41076/setjwt", json);
    ...
    private async void TimerElapsed(object? sender, ElapsedEventArgs? e)
    {
        try
        {
            using var response = await _http.GetAsync("http://127.0.0.1:41076/state");
